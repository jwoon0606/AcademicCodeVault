...
void Speaker::VolumeUp(){
    if(volume == 5) printf("Volume is arleady Max level 5\n");
    else volume += 1;
}

// decrease volume down by 1 level
void Speaker::VolumeDown(){
    if(volume == 1) printf("Volume is arleady MIN level 1\n");
    else volume -= 1;
}
...
void Speaker::ShowStatus(){
    if(OnOff == true) printf("Current volume level is %d.\n",volume);
    else printf("Speaker is off now.\n");
}
